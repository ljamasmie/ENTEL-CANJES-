...
| **Web estática** (`index.html`) | Un solo archivo HTML con datos ya agregados. Se abre en cualquier navegador o se publica con GitHub Pages. |
...
- `index.html` contiene **solo datos agregados** (por día, canal, comercio y monto), pero son datos comerciales del cliente: **mantén el repositorio privado**.
...
├── build_html.py           # Regenera index.html desde el Excel
├── template.html           # Plantilla del dashboard web
├── index.html              # Dashboard web generado (GitHub Pages)
├── logo_entel.png
├── .streamlit/config.toml  # Tema con colores Entel
└── requirements.txt
```

El Excel va en la raíz o en una carpeta `data/`; `.gitignore` impide que se suba.
...
Copia el Excel en la carpeta del proyecto (por ejemplo `ticket_51020.xlsx`).
...
Se abre en http://localhost:8501. Si no hay Excel en la carpeta, se puede subir desde la barra lateral.

**Actualizar la versión web**

```bash
python build_html.py ticket_51020.xlsx
```

Luego haz commit de `index.html`.

## Publicar

- **GitHub Pages:** Settings → Pages → *Deploy from a branch* → rama `main`, carpeta `/ (root)`. Ojo: en repos privados requiere un plan de pago, y la página queda pública aunque el repo sea privado (salvo en GitHub Enterprise con Pages privadas).
